...
@extend_schema_view(
    post=extend_schema(
        ...
        * 계정생성은 `django-backend-starter/apps` 하위에서 `python manage.py createsuperuser --username root` 커맨드를 수행하거나
        ...
        """,
        examples=[
            OpenApiExample(request_only=True, name="로그인 샘플 포맷", value={"username": "root", "password": "1234"})
        ],
    )
)
class AuthLoginAPIView(TokenObtainPairView):
    pass  # TokenObtainPairView의 기본 post 메서드를 사용
...
class AuthRefreshAPIView(TokenRefreshView):
    pass  # TokenRefreshView의 기본 post 메서드를 사용
...
class AuthTokenBlacklistView(TokenBlacklistView):
    pass  # TokenBlacklistView의 기본 post 메서드를 사용
