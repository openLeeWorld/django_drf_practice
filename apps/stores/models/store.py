...
class StoreManger(models.Manager["Store"]):
    ...
    def current_valid(self, store=None):  # 현재 유효한 계약
        today = date.today()
        return self.filter(start_date__gte=today, end_date__lt=today, store=store)

    def recently_expired(self, store):  # 가장 최근에 만료된 계약 순서대로
...
class Store(models.Model):  # 상점 계약
    class StoreType(models.TextChoices):
        FOOD = "food", "배달 음식"
        GROCERY = "grocery", "식료품/가공식품"
        PET_FOOD = "pet_food", "반려동물식품"
    ...
    store_name_display_only = models.CharField(max_length=128, db_comment="직원이 해당 상점을 한 눈에 볼 수 있게 관리하는 필드")
